...
<div class="grid_9" id="cart_table_cont">
<h3 class="title"><?php _e('Please review your order', 'wpsc'); ?></h3>
<table class="checkout_cart">
   ...
   <?php while (wpsc_have_cart_items()) : wpsc_the_cart_item(); ?>
      ...
      <tr class="product_row product_row_<?php echo wpsc_the_cart_item_key(); ?> <?php echo $alt_class;?>">
         ...
         <?php if('' != wpsc_cart_item_image()): ?>
            ...
            <img src="<?php echo wpsc_cart_item_image(get_option('checkout_im_width'), get_option('checkout_im_height')); ?>" alt="<?php echo wpsc_cart_item_name(); ?>" title="<?php echo wpsc_cart_item_name(); ?>" class="product_image" />
         ...
         <td><?php echo curSimble(wpsc_cart_single_item_price()); ?></td>
         <td class="wpsc_product_quantity wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform qty">
               ...
               <input type="text" name="quantity" id="wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>" size="1" value="<?php echo wpsc_cart_item_quantity(); ?>" />
               </div>
               ...
               <input type="submit" class="cart_bt" value="<?php _e('Update', 'wpsc'); ?>" name="submit" />
               <script>
                                          jQuery('.wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?> .down').click(function(){
                                             jQuery('#wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>').fadeOut('fast', function () {
                                                item = (jQuery('#wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>').val() - 0);
                                                
                                                item = item - 1;
                                                
                                                if (item < 1) item = 1;

                                                jQuery('#wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>').val(item);
                                                jQuery('#wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>').fadeIn('fast');
                                             });
                                          });

                                          jQuery('.wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?> .up').click(function(){
                                             jQuery('#wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>').fadeOut('fast', function () {
                                                item = (jQuery('#wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>').val() - 0);
                                                item = item + 1;
         ...
         <td class="wpsc_product_price wpsc_product_price_<?php echo wpsc_the_cart_item_key(); ?>"><span class="pricedisplay"><?php echo curSimble(wpsc_cart_item_price()); ?></span></td>
         ...
         <td class="wpsc_product_remove wpsc_product_remove_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform remove">
               ...
               <input type="hidden" name="key" value="<?php echo wpsc_the_cart_item_key(); ?>" />
               <input type="hidden" name="wpsc_update_quantity" value="true" />
               <input class="cart_bt" type="submit" value="<?php _e('Remove', 'wpsc'); ?>" name="submit" />
            </form>
         </td>
      </tr>
   ...
   if(wpsc_uses_coupons()): ?>
      ...
      <tr class="wpsc_coupon_row">
         <td class="coupon_code" colspan="6">
            <div class="enter-code"><?php _e('Enter coupon code', 'wpsc'); ?> :</div>
            <form  method="post" action="<?php echo get_option('shopping_cart_url'); ?>">
               ...
               <button class="button-2" type="submit" value="<?php _e('Update', 'wpsc') ?>"><span><?php _e('Update', 'wpsc') ?></span></button>
            </form>
         </td>
      </tr>
  ...
  <?php if(wpsc_uses_shipping()): ?>
   ...
   <?php if(wpsc_uses_shipping()) : ?>
      ...
      <h3  class=" top-20 "><?php _e('Calculate Shipping Price', 'wpsc'); ?></h3>
        ...
        <?php if(wpsc_checkout_form_is_header() == true){
               ...
               <tr <?php echo wpsc_the_checkout_item_error_class();?>>
                  <td <?php wpsc_the_checkout_details_class(); ?> colspan='2'>
                     <h3  class=" top-20 "><?php echo wpsc_checkout_form_name();?></h3>
                  </td>
               </tr>
            ...
            <?php
               ...
               "<div class='wpsc_email_address'>
                  <p class='" . wpsc_checkout_form_element_id() . "'>
                     <h3 class='wpsc_email_address  top-20'>
...
?>
      ...
      <?php if(!wpsc_has_tnc()) : ?>
            ...
            <?php endif; ?>
      		...
      		<input type='submit' id="purchase_button" style="display:none" value='<?php _e('Purchase', 'wpsc');?>' name='submit' class='make_purchase wpsc_buy_button'/>
      </form>
</div>
</div><!--close checkout_page_container-->
</div>
<div class="grid_3" style="position:relative;" id="total_container">
   
   <div id="totable">
   <table  class='wpsc_checkout_table table-4'>
      <?php if(wpsc_uses_shipping()) : ?>
	      <tr class="header">
	         <th class='wpsc_total_price_and_shipping'colspan='2'>
	            <?php _e('Review and purchase','wpsc'); ?>
	         </th>
	      </tr>
   ...
   <tr class='total_price grand_total'>
      <td class='wpsc_totals'>
...
<!-- div for make purchase button -->
      <div class='wpsc_make_purchase'>
         <span>
               <a id="special_button" class="input make_purchase"><?php _e('Purchase', 'wpsc');?></a>
         </span>
      </div>
      <script>
      	jQuery('#special_button').click(function(){
      		jQuery('#purchase_button').trigger('click');
      	});
      </script>
</div>
<div class='clear'></div>
...
<script>
   if (document.documentElement.clientWidth > 769) { checkSwitcher('#totable'); }
</script>
